/**
 * @file position.h
 * @brief Chess position representation and move generation
 * 
 * Contains the Position class for representing chess positions,
 * including bitboard-based board state, move generation, and
...
/**
 * @brief Pseudorandom number generator (from Stockfish)
...
public:
	...
	// Generate pseudorandom number with few set bits
	template<typename T> 
	T sparse_rand() {
...
/**
 * @brief Zobrist hashing for position identification
 */
namespace zobrist {
...
/**
 * @brief Stores position information that cannot be recovered on undo
 */
struct UndoInfo {
	// Bitboard of squares where pieces have moved from/to (for castling legality)
	Bitboard entry;
	...
	// Preserves the entry bitboard across moves
 ...
 * 
 * Uses bitboard representation for efficient move generation
 * and evaluation. Supports make/unmake for search algorithms.
 */
class Position {
private:
	// Bitboard of each piece type
	Bitboard piece_bb[NPIECES];
	
	// Mailbox representation (piece at each square)
...
// Returns a bitboard containing all pieces of a given color
template<Color C> 
...
// Returns a bitboard of pieces attacking a square
template<Color C> 
...
Move* Position::generate_legals(Move* list) {
	...
	// Calculate danger squares (attacked by enemy)
	...
	// Non-pinned diagonal slider moves
	...
	// Non-pinned orthogonal slider moves
	...
	// Non-pinned pawn moves (not on promotion rank)
...
/**
 * @brief Convenience wrapper for iterating over legal moves
 * @tparam Us Color to generate moves for
 */
template<Color Us>
class MoveList {
public:
