/**
 * @file types.h
 * @brief Core type definitions for chess board representation
 * 
 * This file contains fundamental types used throughout the chess engine,
 * including piece types, colors, squares, bitboards, and move representations.
 */
...
// Board directions for piece movement
enum Direction : int {
...
// Number of piece types (excluding color)
...
// Total number of piece values (including NO_PIECE)
const size_t NPIECES = 15;

// Chess pieces with color information
enum Piece : int {
...
// Extracts the piece type from a piece
constexpr PieceType type_of(Piece pc) {
	return PieceType(pc & 0b111);
}

// Extracts the color from a piece
constexpr Color color_of(Piece pc) {
	return Color((pc & 0b1000) >> 3);
}

// Bitboard type for efficient board representation (64 bits = 64 squares)
typedef uint64_t Bitboard;

// Number of squares on the board
const size_t NSQUARES = 64;

// Board squares in little-endian rank-file mapping
enum Square : int {
...
// File indices (columns a-h)
enum File : int {
	AFILE, BFILE, CFILE, DFILE, EFILE, FFILE, GFILE, HFILE
};	

// Rank indices (rows 1-8)
enum Rank : int {
	RANK1, RANK2, RANK3, RANK4, RANK5, RANK6, RANK7, RANK8
};

// Lookup table for square names in algebraic notation
extern const char* SQSTR[65];

// Precomputed bitboard masks for files, ranks, and diagonals
...
// Prints a bitboard to standard output for debugging
extern void print_bitboard(Bitboard b);

// Constants for pop_count bit manipulation
...
// Returns number of set bits (faster for sparse bitboards)
extern int sparse_pop_count(Bitboard x);

// Pops and returns the least significant bit
extern Square pop_lsb(Bitboard* b);

// De Bruijn sequence for bit scanning
...
// Shifts a bitboard in a particular direction (no wrapping)
...
// Returns the actual rank from a given side's perspective
template<Color C>
constexpr Rank relative_rank(Rank r) {
	return C == WHITE ? r : Rank(RANK8 - r);
}

// Returns the actual direction from a given side's perspective
template<Color C>
 ...
 * 
 * Internal representation uses 16 bits:
...
// Adds all moves of the form (from, s) to the move list
...
// Specialization for quiet promotions
template<>
...
// Specialization for capture promotions
template<>
...
// Combined castling mask for all pieces
